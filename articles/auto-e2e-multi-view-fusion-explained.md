...
最後に、取り出した情報を attention weight で重み付けし、可視カメラで平均して、1つの BEV 特徴マップ `[B, 1440, 7, 7]` を作ります。
...
View Fusion の出力 `[B, 1440, 7, 7]` は車両周囲のシーンを1つにまとめた表現です。ここから2つの予測を並列に行います。1つは走行軌跡の直接予測（DrivingPolicy）、もう1つは未来のシーン特徴の予測（FutureState）です。この2つは独立した出力ですが、学習時にはそれぞれ異なる Loss を生成し、合算してネットワーク全体を更新します。
...
# [2, 1440, 7, 7] → [2, 2880, 7, 7]
```

チャンネル数を 1440 → 2880 に2倍に拡張します。kernel=3 の畳み込みなので、隣接する空間位置の情報を混ぜながらチャンネルを増やしています。チャンネル数を増やすのは、この後 4 つの時間ステップに分割するための準備です。
...
# [2, 2880, 7, 7] → [2, 5760, 7, 7]
```

2880 → 5760 にさらに2倍に拡張します。5760 = 1440 × 4 であり、4つの未来時刻それぞれに 1440 チャンネルの特徴を割り当てるための準備が整いました。

#### Step 3: 時間方向に4分割

```python
future_visual_features = torch.chunk(future_features, chunks=4, dim=1)
# [2, 5760, 7, 7] → ([2, 1440, 7, 7], [2, 1440, 7, 7], [2, 1440, 7, 7], [2, 1440, 7, 7])
```

5760 チャンネルをチャンネル方向に均等に4分割します。各チャンクが1つの未来時刻に対応します:

```
chunk 0: [2, 1440, 7, 7] → t+1.6s の予測特徴
chunk 1: [2, 1440, 7, 7] → t+3.2s の予測特徴
chunk 2: [2, 1440, 7, 7] → t+4.8s の予測特徴
chunk 3: [2, 1440, 7, 7] → t+6.4s の予測特徴
```

各チャンクは現在の統合特徴 `[B, 1440, 7, 7]` と同じ形状です。つまり「今のシーン表現」と「1.6秒後のシーン表現（予測）」が同じ形式なので、直接比較が可能です。
...
ピクセルレベルの未来予測（次のフレームの画像を生成する）は計算が重く、照明変化やテクスチャの細部に左右されやすい問題があります。例えば、曇りの日と晴れの日でピクセル値は大きく異なりますが、運転判断にとっての意味（前に車がいる）は同じです。

JEPA（Joint Embedding Predictive Architecture, LeCun 2022）の考え方では、意味的に重要な情報だけを抽出した特徴空間で予測します。特徴空間ではピクセル値の違いは既に抽象化されているため、「前に車がいて近づいてきている」という本質的な変化だけを予測すればよくなります。

---

### FutureState が DrivingPolicy の学習を助ける仕組み

FutureState は auxiliary task（補助タスク）として機能します。

trajectory loss だけでネットワーク全体を学習する場合を考えてみます。Loss から勾配が逆伝播して Backbone に到達するまでに、DrivingPolicy の MLP 3層 → flatten → Conv → View Fusion → Feature Fusion → Backbone と多数のレイヤーを経由します。勾配はレイヤーを経由するたびに薄まる（勾配消失）傾向があるため、Backbone が受ける学習信号は弱くなります。

feature reconstruction loss は View Fusion の直後から分岐する FutureState に直接かかるため、Backbone と View Fusion に対する勾配の経路が短くなります。「シーンの時間変化を予測できるだけの情報を抽出しろ」という直接的な学習圧力が Backbone にかかり、より豊かな特徴抽出を促します。
...
学習後、FutureState の予測誤差をリアルタイムで監視することで、モデルの不確実性を推定できます。

通常の走行シーン（直線道路、通常の交通量）では、FutureState の予測と実測の差は小さいはずです。しかし急な飛び出し、工事による車線規制、見慣れない天候条件などに遭遇すると、予測と実測が大きく乖離します。この乖離の大きさを数値化すると:

```python
prediction_error = MSE(predicted_features, actual_features)
# prediction_error が閾値を超えた → 「モデルの予測が外れている」
# → 自信度を下げる / 安全側に制御する / 人間に介入を求める
```

つまり FutureState は、学習時には Backbone の特徴品質を向上させる補助タスクとして、推論時には安全性モニタリングの信号源として、二重に機能するモジュールです。
