---
title: "自動運転モデル開発に Flyte・MLflow・Kueue を導入した理由"
emoji: "🔁"
type: "tech"
...
AutoE2E のフルパイプラインでは、2つの異なるデータセット（L2D と NVIDIA Physical AI）を独立に取り込んで前処理し、結果を合流させて学習に渡します。
...
g6e.4xlarge は人気インスタンスで、オンデマンドでの確保が困難な場合があります。
On-Demand Capacity Reservation（ODCR）で容量を確保し、Karpenter の NodePool を ODCR の AZ（us-west-2b）に固定しています。
ODCR で容量を保証し、Kueue でジョブの入場順序を制御し、Karpenter でノードをプロビジョニングする3層構成により、「GPU は確実にあるが、使っていないときはコストゼロ」を実現しています。
...
Woven by Toyota や Wayve が Flyte を選択しているのは、ペタバイト規模のセンサーデータと安全性証明の要求のもとで、型安全性、再現性、リネージュがオーケストレーターの最優先要件になるためです。
