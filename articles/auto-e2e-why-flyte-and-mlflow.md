...
基本的にいつも `wf_full_pipeline` を起動します。
URI をコピペして手動で繋ぐ必要はありません。
Flyte のキャッシュが「どのステージを再実行すべきか」を自動判断するためです。

`cache=True` のタスクは、入力とコードが前回と同じならキャッシュヒットで数秒でスキップされます。
`wf_full_pipeline` を毎回起動しても、変更があったステージだけが実際に計算を行い、残りは即座に完了します。
変更の種類に応じて異なるワークフローを使い分ける必要はありません。
いくつかのシナリオでどう動くかを示します。

**学習のハイパーパラメータを探索したい場合**

backbone を swin_v2_tiny に固定したまま、lr を 1e-4、3e-4、1e-3 で試したいとします。
`wf_full_pipeline` を lr だけ変えて3回起動します。
データ取り込みと前処理は入力が同じなのでキャッシュヒットし数秒で完了。
`train_il` は lr が異なるためキャッシュミスし、学習だけが走ります。
前処理を3回走らせる無駄は発生しません。

**データセットの Hz 数や解像度を変えたい場合**

`data_processing` タスクのコードを修正して `hz=20`、`image_size=512` に変更し、`pyflyte register` で再登録します。
`wf_full_pipeline` を起動すると、`data_ingest` は入力が変わっていないのでキャッシュヒット。
`data_processing` はコードが変わったため `cache_version` が無効化され、新しいパラメータで再実行されます。
その新しい shards が自動的に `train_il` に渡り、学習と評価が走ります。
URI を手でコピーする操作は発生しません。

**モデルアーキテクチャを変更した場合（Head の追加など）**

`AutoE2E` に新しい出力 Head を追加した場合、`train_il` タスクのコンテナイメージが変わります。
Docker イメージを再ビルドして ECR に push し、`pyflyte register` で再登録。
`wf_full_pipeline` を起動すると、データ取り込みと前処理はキャッシュヒット。
`train_il` はコンテナイメージが変わったため再実行されます。
アーキテクチャを変更しても、データ準備の待ち時間はゼロです。

**評価メソッドを変更した場合**

ADE/FDE に加えて Comfort メトリクス（Jerk、横加速度）を追加したい場合、`evaluate_il_policy` タスクのコードを修正してイメージを再ビルドします。
`wf_full_pipeline` を起動すると、データ取り込み、前処理、学習は全てキャッシュヒット。
`evaluate_il_policy` だけが新しいコードで再実行されます。
3時間かけた学習をやり直す必要はありません。

**新しいデータセットを追加した場合**

`Dataset` Enum に新しい値（たとえば `WAYMO_OPEN`）を追加し、対応する ingest / processing コードを書いて再登録します。
`wf_full_pipeline` を `dataset=WAYMO_OPEN` で起動すると、全ステージがキャッシュミスして最初から実行されます。
一方、同じタイミングで `dataset=L2D` を起動しても、L2D 側はキャッシュヒットで前処理がスキップされるため GPU を奪い合いません。

このように、開発者は常に `wf_full_pipeline` を起動するだけで、「どのステージを再実行すべきか」は Flyte のキャッシュが自動判断します。
変更した箇所だけが再計算され、それ以外はスキップされます。
